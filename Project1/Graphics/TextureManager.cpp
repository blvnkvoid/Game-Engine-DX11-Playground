...
ID3D11ShaderResourceView* TextureManager::GetTexture(std::wstring filename, ID3D11DeviceContext* context) {
    ...
    if (filename.find(L".dds") != std::wstring::npos) {
        hr = DirectX::CreateDDSTextureFromFile(m_device, filename.c_str(), nullptr, &srv);
        
    }
    else {
        hr = DirectX::CreateWICTextureFromFileEx(
            m_device, context, filename.c_str(), 0,
            D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_RENDER_TARGET,
            0, D3D11_RESOURCE_MISC_GENERATE_MIPS, DirectX::WIC_LOADER_DEFAULT, nullptr, &srv
        );
    }
    ...
    if (SUCCEEDED(hr) && srv != nullptr) {
        ...
        wchar_t buf[256];
        swprintf_s(buf, L"ERROR: Failed to load texture: %s (HRESULT 0x%08X)\n", filename.c_str(), hr);
